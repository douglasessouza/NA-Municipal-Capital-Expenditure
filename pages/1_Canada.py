"""Canada page — 62 municipalities, FY2025/26, two-axis tiering, CAD."""
...
st.title("Canada")
st.caption(
    "62 municipalities, FY2025–26, two-axis tiering (capex per capita + "
    "strategic-vision score). Currency: CAD."
)
...
def _apply_filters(
    ...
    out = out[out["state_province"].isin(provinces)] if provinces else out.iloc[0:0]
    out = out[out["fiscal_year"].isin(fys)] if fys else out.iloc[0:0]
    lo, hi = pop_range
...
tier_counts = filtered["tier"].value_counts().reindex(CANADA_TIERS, fill_value=0)
tier_cols = st.columns(len(CANADA_TIERS))
for col, tier in zip(tier_cols, CANADA_TIERS):
...
if map_df.empty:
    ...
    view_state = pdk.ViewState(
        latitude=56.0, longitude=-96.0, zoom=3, pitch=0, bearing=0
    )
    ...
    legend_cols = st.columns(len(CANADA_TIERS))
    for col, tier in zip(legend_cols, CANADA_TIERS):
        col.markdown(
...
if filtered.empty:
    ...
    with grid_top[0]:
        ...
        # Plotly draws bars bottom-up; reverse so highest is at top of each panel
        ...
        fig1 = px.bar(
            ...
            category_orders={"tier": list(CANADA_TIERS)},
        )
        fig1.update_layout(
    ...
    with grid_top[1]:
        ...
        # Read comparison-view selection from session state and overlay
        # vertical lines for any selected city present in the filtered set.
        selected_compare_labels = st.session_state.get("ca_compare_select", [])
    ...
    grid_bot = st.columns(2)

    # --- Chart 3: Province averages (population-weighted) ------------------
    with grid_bot[0]:
        st.markdown("**Province averages (population-weighted)**")
        st.caption(
            f"Average capex per capita ({CURRENCY}) by province, weighted by "
            "population."
        )
        agg = (
        ...
        st.caption(
            "Provinces marked with `*` contain a single municipality — the "
            "value reflects that city, not a regional average."
        )
        if truncated:
            st.caption(f"Showing top 25 of {len(agg)} provinces.")

    # --- Chart 4: Tier scatter (Canada-only) -------------------------------
    with grid_bot[1]:
        st.markdown("**Tier scatter — capex score vs vision score**")
        st.caption(
            "Two-axis tier rubric. X = capex score (0–4), Y = vision score "
            "(0–4). Quadrant shading reflects the rubric's intended tier."
        )
        scatter_df = chart_df.dropna(
            subset=["capex_score", "vision_score_norm"]
        ).copy()
        if scatter_df.empty:
            st.info("No rows have both capex and vision scores in the filter.")
        else:
            rng = np.random.default_rng(seed=42)
            n = len(scatter_df)
            scatter_df["x_jitter"] = (
                scatter_df["capex_score"].astype(float)
                + rng.uniform(-0.15, 0.15, size=n)
            )
            scatter_df["y_jitter"] = (
                scatter_df["vision_score_norm"].astype(float)
                + rng.uniform(-0.15, 0.15, size=n)
            )

            fig4 = go.Figure()

            # Quadrant shading. Boundary at score = 2 (mid of 0-4).
            quadrants = [
                # (x0, y0, x1, y1, tier)
                (-0.5, -0.5, 2.0, 2.0, "Niche Players"),
                (2.0, -0.5, 4.5, 2.0, "Challengers"),
                (-0.5, 2.0, 2.0, 4.5, "Visionaries"),
                (2.0, 2.0, 4.5, 4.5, "Leaders"),
            ]
            for x0, y0, x1, y1, tier in quadrants:
                rgb = TIER_COLORS_RGB[tier]
                fig4.add_shape(
                    type="rect",
            ...
            for tier in CANADA_TIERS:
                tier_rows = scatter_df[scatter_df["tier"] == tier]
                if tier_rows.empty:
                    continue
                fig4.add_trace(
                    go.Scatter(
                        x=tier_rows["x_jitter"],
                        y=tier_rows["y_jitter"],
                        mode="markers",
                        name=tier,
                        marker=dict(
                            size=10,
                        ...
                        customdata=tier_rows[
                            ["municipality", "state_province",
                             "capex_score", "vision_score_norm", "tier"]
                        ].values,
                        hovertemplate=(
                            "<b>%{customdata[0]}</b>, %{customdata[1]}<br>"
                            "Capex score: %{customdata[2]}<br>"
                            "Vision score: %{customdata[3]}<br>"
            ...
            fig4.update_layout(
                ...
                yaxis=dict(
                    title="Vision score (normalized 0–4)",
                    range=[-0.5, 4.5],
                    dtick=1,
                ),
                margin=dict(l=10, r=10, t=10, b=10),
...
if not selected_labels:
    ...
    if len(fys_in_sel) > 1:
        st.warning(
            "You're comparing municipalities from different fiscal years "
            f"({sorted(fys_in_sel)}). Numbers may not be directly comparable."
        )
    ...
    def _fmt_float(v: object, digits: int = 1) -> str:
        if pd.isna(v):
            return "N/A"
    ...
    for col, (_, row) in zip(cols, sel.iterrows()):
        with col:
            ...
            rows_to_show: list[tuple[str, str]] = [
                ...
                ("Vision score raw (0-6)", _fmt_float(row["vision_score_raw"])),
                ("Vision score norm (0-4)", _fmt_float(row["vision_score_norm"])),
...
st.caption(f"Showing **{len(filtered):,}** of {len(df):,} Canadian municipalities.")
...
st.download_button(
    ...
    data=csv_bytes,
    file_name="municipal_capex_canada.csv",
    mime="text/csv",
)
